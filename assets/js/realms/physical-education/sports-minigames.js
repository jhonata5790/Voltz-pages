(function initializeSportsMinigames(global) {
  ...
  function addFootballVoltzThreat(g, amount, now = performance.now(), reason = "") {
...
: `Derrota ${g.score} a ${g.rivalScore}. O rival puniu as decisões apressadas.`);
  ...
  function getFootballRivalCoverAssignments(g, owner, rivalOutfield, presserId, now = performance.now()) {
...
: blocked
  ...
  // Vôlei
  // -------------------------------------------------------
  // Volei V0.3 · rally continuo em tempo real.
  ...
  function volleyballAttackerRead(g, team, setter, candidate, input = null) {
  ...
  function volleyballVoltzSet(g, player) {
    ...
    const targetX = clamp(attacker.x + (input.moving ? input.x * 2 : 0), 16, 84);
    const targetY = 56.5;
  ...
  function volleyballRivalContactReady(player, ball, kind) {
    ...
    openPanelShell(
      ...
      `<div class="sports-game-card volleyball-dynamic-card">
        ...
        <div class="volleyball-dynamic-hud">
          ...
          <div class="sports-help">O controle troca automaticamente para quem entra na jogada. No 2º toque, sua direção de movimento indica o setor e o jogo escolhe o atacante mais apto dentro dele.</div>
        </div>
      </div>`
    );
    syncVolleyballDynamicDom(performance.now());
